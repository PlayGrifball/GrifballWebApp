...
namespace GrifballWebApp.Server.Sorting;

// Original code from Khalid Abuhakmeh
// https://khalidabuhakmeh.com/sort-data-with-aspnet-core-and-query-strings
public static class OrderByExtensions
{
    public static IQueryable<T> OrderBy<T>(this IQueryable<T> query, PaginationFilter filter)
    {
        if (filter.SortDirection is null || filter.SortColumn is null)
            return query;

        var isAsc = filter.SortDirection is SortDirection.Asc;

        if (isAsc)
        {
            return query.OrderBy(filter.SortColumn);
        }
        else
        {
            return query.OrderBy("-" + filter.SortColumn);
